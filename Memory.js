...
//Splash screen
    ...
    for (let c=0 ;c<cureentBlocks.length ; c++){
        cureentBlocks[c].addEventListener("click", function(){this.classList.add("flipped");
        ...
        if (flipNumber.length == 2 ) {
            ...
            if (timerInitialiazer==0) {
             ...
             timerCounter();pauseGame.classList.remove("displayImage") ;
    ...
    function stopClicking() {
        ...
        setTimeout(()=>{
          allBlocks.classList.remove("no-clicking")  },duration+30);
    }
        ...
        secs = ("0"+secs).slice(-2);
            ...
            if (secs>=60 ) {
              secs = secs-60;
                ...
                mins++;
                mins = ("0"+mins).slice(-2);
        secs = ("0"+secs).slice(-2);clockCounter.classList.add("alert");
            ...
            restartGameVar.addEventListener("click",function(){clearInterval(startCounting);timerInitialiazer=0;mins=0;secs=0;clockCounter.textContent = "0"+mins +":" + "0"+secs ;
                                            allBlocks.classList.remove("no-clicking");
                                            pauseGame.classList.remove("displayImage") ;
                                 resumeGame.classList.add("displayImage");
                                            })
             pauseGame.addEventListener("click",function(){clearInterval(startCounting);timerInitialiazer=1;
                                 resumeGame.classList.remove("displayImage") ;
                                 this.classList.add("displayImage");
                                 allBlocks.classList.add("no-clicking")
                                   });
        resumeGame.addEventListener("click",function (){timerInitialiazer=1;
                                  timerCounter();
                                 pauseGame.classList.remove("displayImage") ;
                                 this.classList.add("displayImage");
                                 allBlocks.classList.remove("no-clicking");
    ...
    document.getElementById("play-again").onclick=function(){
        winnerPop.classList.remove("appear");
        clearInterval(startCounting);timerInitialiazer=0;mins=0;secs=0;clockCounter.textContent = "0"+mins +":" + "0"+secs ;
        allBlocks.classList.remove("no-clicking");
        pauseGame.classList.remove("displayImage") ;
        resumeGame.classList.add("displayImage");
        restartGame();
